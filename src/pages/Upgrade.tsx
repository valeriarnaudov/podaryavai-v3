...
export default function Upgrade() {
    ...
    if (plansLoading) {
        return (
            <div className="min-h-screen bg-background dark:bg-slate-900 flex flex-col items-center justify-center">
                <Loader2 className="w-10 h-10 text-accent animate-spin mb-4" />
                <p className="text-slate-500 dark:text-slate-400 font-medium">{t('upgrade.loadingPlans')}</p>
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 relative pb-20">
            {/* Header / Nav */}
            <header className="absolute top-0 w-full p-4 sm:p-6 flex items-center justify-between z-10">
                <button
                    onClick={() => navigate(-1)}
                    className="p-3 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 rounded-full shadow-sm text-slate-700 dark:text-slate-200 transition"
                >
                    <ArrowLeft className="w-6 h-6" />
                </button>
                {user && (
                    <div className="bg-white dark:bg-slate-800 px-4 py-2 rounded-full shadow-sm border border-slate-100 dark:border-slate-700 flex items-center space-x-2">
                        <span className="text-xs text-slate-500 dark:text-slate-400">{t('upgrade.currentPlan')}</span>
            ...
            <section className="pt-24 pb-12 px-6 flex flex-col items-center text-center">
                ...
                <motion.p
                    ...
                    className="text-slate-500 dark:text-slate-400 text-lg max-w-md"
                >
                    {t('upgrade.subtitle')}
                </motion.p>
            </section>
