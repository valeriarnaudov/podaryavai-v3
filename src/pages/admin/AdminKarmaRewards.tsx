...
export default function AdminKarmaRewards() {
    ...
    return (
        <div className="space-y-6 max-w-4xl pb-10">
            <header className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm sticky top-4 z-10 space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 shrink-0">
                            {!isEditing && (
                                <select 
                                    ...
                                    className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 cursor-pointer shadow-sm transition-all"
                                >
                            ...
                            {isEditing ? (
                                <>
                                    <button onClick={() => setIsEditing(null)} className="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-100 rounded-xl transition-colors">Cancel</button>
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl w-fit">
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'rewards' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Store Rewards
                    </button>
                    <button
                        onClick={() => setActiveTab('history')}
                        className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'history' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Claim History
                    </button>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-accent/20 border-l-4 border-l-accent animate-fade-in text-sm text-left align-middle mx-auto">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select value={editForm.reward_type || 'PLAN_UPGRADE'} onChange={e => setEditForm({ ...editForm, reward_type: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-accent focus:bg-white">
                        ...
                        {editForm.reward_type === 'ADD_FREE_DELIVERIES' && (
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Amount to Add</label>
                                <input type="number" value={editForm.reward_metadata?.amount || 1} onChange={e => setEditForm({ ...editForm, reward_metadata: { ...editForm.reward_metadata, amount: parseInt(e.target.value) || 1 } })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-accent focus:bg-white" />
                            </div>
                        )}

                        {editForm.reward_type === 'CUSTOM_SERVICE' && (
                            <div className="col-span-2 md:col-span-1">
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Instructions for Admin (Metadata)</label>
                                <input type="text" placeholder="e.g. Schedule 30m call" value={editForm.reward_metadata?.instructions || ''} onChange={e => setEditForm({ ...editForm, reward_metadata: { ...editForm.reward_metadata, instructions: e.target.value } })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-700 outline-none focus:border-accent focus:bg-white" />
                            </div>
                        )}
            ...
            {!isEditing && rewards.length > 0 && (
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-2 text-xs font-semibold text-slate-400">
                                        <span className="bg-slate-50 px-2 py-1 rounded-md text-blue-600">
                            ...
                            <div className="flex items-center gap-2 self-start md:self-auto shrink-0">
                                <label className="flex items-center cursor-pointer mr-2" title={reward.is_active ? "Deactivate" : "Activate"}>
                                    ...
                                    <span className={`w-9 h-5 flex items-center rounded-full p-1 duration-300 ease-in-out ${reward.is_active ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                                        <div className={`bg-white w-3 h-3 rounded-full shadow-md transform duration-300 ease-in-out ${reward.is_active ? 'translate-x-4' : 'translate-x-0'}`} />
                                    </span>
                                </label>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                    <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                        ...
                        <p className="text-sm text-slate-500">Recent rewards claimed by users.</p>
                    </div>
                    {loadingHistory ? (
                        <div className="flex justify-center p-10"><Loader2 className="w-6 h-6 animate-spin text-slate-400" /></div>
                    ) : historyLogs.length === 0 ? (
                        <div className="text-center py-12"><p className="text-slate-500">No rewards claimed yet.</p></div>
                    ) : (
                        <div className="divide-y divide-slate-100">
                            {historyLogs.map(log => {
                                ...
                                return (
                                    <div key={log.id} className="p-4 hover:bg-slate-50 flex items-start sm:items-center justify-between gap-4 flex-col sm:flex-row transition-colors">
                                        <div>
                                            <p className="font-bold text-slate-800">{log.description}</p>
                                            <div className="flex flex-wrap items-center gap-2 mt-1 -mb-1 text-sm text-slate-500">
                                                <span className="bg-slate-100 px-2 py-0.5 rounded-md font-medium text-slate-600 truncate max-w-[200px] sm:max-w-xs" title={userDisplay}>{userDisplay}</span>
                                                <span>•</span>
